...
const getPeripheralVessel = (peripheralVessel = {}) => {
  const {
    symptoms,
    dorsalisPedisRight,
    dorsalisPedisLeft,
  ...
  const getABITBI = () => {
    ...
    let scenario2 = false
    if (
      (ABIRight && TBIRight && (ABIRight > 1.3 && TBIRight < 0.6)) ||
  ...
  const isDorsalisPedis =
    [dorsalisPedisRight, dorsalisPedisLeft].indexOf('missing') !== -1
  const isABIOrTBI = getABITBI()
  return isRestPain || isDorsalisPedis || isABIOrTBI
}

const getPeripheralNerve = (peripheralNerve = {}) => {
  const {
    symptom,
    pressureSense,
    vibrationSense,
  ...
  const combBad = property =>
    isBad(get(property, 'left')) || isBad(get(property, 'right'))
  ...
  const acupunctureSenceFilter = array.filter(item =>
    isBad(get(acupunctureSence, `${item}.${item}-normal`)),
  )
  const badFilter = [
    isPressureSense,
    isVibrationSense,
    isThalposis,
    isAnkleJerk,
    acupunctureSenceFilter.length,
  ].filter(o => o)
  return (
    (isExistedSymptom && !!badFilter.length) ||
    (!isExistedSymptom && badFilter.length > 1)
  )
}
...
const getBoneAndJoint = (boneAndJoint = {}, skinConditions) => {
  ...
  return (
    ...
    !!twoSide.filter(
      item =>
        get(boneAndJoint, `deformity${item}.had`) &&
        footBad.filter(o => get(boneAndJoint, `deformity${item}.${o}`)).length,
    ).length
  )
}
...
export const highRiskFoot = data => {
  ...
  const isFootUlcer =
    get(medicalHistory, 'had') && get(medicalHistory, 'footUlcer')
  const isAmputated =
    get(medicalHistory, 'had') && get(medicalHistory, 'amputation')
  ...
  const level0 =
    !isFootUlcer &&
    !isAmputated &&
    !isBadForPeripheralVessel &&
    !isBadForPeripheralNerve &&
    !isFootDeformity
  const level1 =
    !isFootUlcer &&
    !isAmputated &&
  ...
  const level2 =
    !isFootUlcer &&
    !isAmputated &&
    (isFootDeformity || (isBadForPeripheralVessel && isBadForPeripheralNerve))
