...
const getFilteredPatients = (appointedPatients, day, hour) => {
  ...
  let currentHourOfDay =
    hourInt !== 0 ? hourInt : moment(moment().subtract(dayInt, 'days')).hour()
  ...
  if (currentDay === 1 && amOrPm === 'morning') {
    filteredPatients = appointedPatients.filter(o => {
      return (
        moment(o.appointmentTime).isSame(
          moment().subtract(3 + dayInt, 'days'),
          'day',
        ) ||
        moment(o.appointmentTime).isSame(
          moment().subtract(4 + dayInt, 'days'),
          'day',
        )
      )
    })
  ...
  if (currentDay === 4 && amOrPm === 'morning') {
    filteredPatients = appointedPatients.filter(o => {
      return moment(o.appointmentTime).isSame(
        moment().subtract(3 + dayInt, 'days'),
        'day',
      )
    })
...
const getOutPatientAmOrPm = (healthCareTeamId, appointmentTime) => {
  ...
  return healthCareTeamMap[healthCareTeamId][
    moment(appointmentTime).isoWeekday()
  ]
}
...
const getAfterOutpatientPatients = async (day, hour) => {
  ...
  let startAt = moment(moment().subtract(parseInt(day) + 6, 'days')).startOf(
    'day',
  )._d
 ...
 */
const checkWhetherExistsBGRecords = async (
  patientId,
  outPatientAmOrPm,
  appointmentTime,
  day,
...
const assembleMsgsAndPubsub = async (filteredOutpatientPatients, day) => {
  ...
  for (let i = 0; i < filteredOutpatientPatients.length; i++) {
    let {
      ...
      healthCareTeamId,
      appointmentTime,
    } = filteredOutpatientPatients[i]

    let messageWordsAndSourceType = {}

    let outPatientAmOrPm = getOutPatientAmOrPm(
      healthCareTeamId,
      appointmentTime,
    )
    let bgRecords = await checkWhetherExistsBGRecords(
      patientId,
      outPatientAmOrPm,
      appointmentTime,
      day,
    ...
    if (bgRecords && bgRecords.length > 0) {
      let whetherPairingRecords = await checkWhetherExistsParingMeasurements(
        bgRecords,
      )
      let triggerResultPageWords = checkWhetherExistsResultEvents(bgRecords)
      if (!triggerResultPageWords) {
        let medicineCaseModifySituation = await checkWhetherModifiedMedicines(
          patientId,
        )
        messageWordsAndSourceType = getMessageTextWithBgRecords(
          whetherPairingRecords,
          medicineCaseModifySituation,
        )
      } else {
        continue
      }
      ...
      messageWordsAndSourceType = getMessageTextNoBgRecords(
        whetherUseBg1,
        medicineCaseModifySituation,
      )
    }

    let messageObject = await assembleMessageObject(
      patientId,
      messageWordsAndSourceType,
    )
  ...
  if (messageArray.length > 0) {
    ...
    return (
      '插入needleChatMessage表中 ' + insertResult.insertedCount + ' 条数据！'
    )
  }
}
...
const checkWhetherModifiedMedicines = async patientId => {
  ...
  if (medicineCaseRecords.length) {
    let modifiedMedicine = medicineCaseRecords[0].caseContent.prescription.medicines.filter(
      o => o.status === 'modify' || o.status === 'add' || o.status === 'stop',
    )
...
const checkWhetherExistsParingMeasurements = async bgRecords => {
  ...
  for (let i = 0; i < bgRecords.length; i++) {
    for (let j = 0; j < i; j++) {
      if (
        bgRecords[j].measurementTime ===
          periodTextMap[bgRecords[i].measurementTime] &&
        moment(bgRecords[i].measuredAt).isSame(
          moment(bgRecords[j].measuredAt),
          'day',
        )
      ) {
        result = true
...
const checkWhetherExistsResultEvents = bgRecords => {
  ...
  let filteredBgRecords = bgRecords.filter(
    bgRecord =>
 ...
 * @returns {Object}
 */
const getMessageTextWithBgRecords = (
  whetherPairingRecords,
  medicineCaseModifySituation,
) => {
  ...
  if (whetherPairingRecords) {
    if (medicineCaseModifySituation) {
      text =
        '看您最近血糖不错，新调整方案还挺适合您的，请遵医嘱继续保持测量，有什么问题请随时和我们沟通。'
      contentCode = 'AOP_5'
    } else {
      text =
        '看您最近血糖控制不错，看来您在饮食或者运动方面很是注意，您觉得对您血糖控制帮助最大的是什么呢？'
      contentCode = 'AOP_6'
    }
  } else {
    if (medicineCaseModifySituation) {
      text =
        '医生最近给您调整了用药方案，建议您配对监测血糖，医生会根据您一餐前后血糖的变化来评估药物以及饮食是否合理，配对测量对您来说有困难吗？'
      contentCode = 'AOP_7'
    } else {
      text =
        '您的测量结果我已经收到，但医生建议您配对测量血糖，这样更有利于帮您判断饮食、运动或者药物存在的问题，您觉得配对测量有困难吗？'
      contentCode = 'AOP_8'
    }
  }
  return {
 ...
 * @returns {Object}
 */
const getMessageTextNoBgRecords = (
  whetherUseBg1,
  medicineCaseModifySituation,
) => {
  ...
  if (whetherUseBg1) {
    if (medicineCaseModifySituation) {
      text =
        '您好，想看一下新调整用药方案对您血糖控制的情况，但是门诊后没有收到您的血糖记录，请问您在测量上有什么困难吗？'
      contentCode = 'AOP_1'
    } else {
    ...
    if (medicineCaseModifySituation) {
      text =
        '您好，想看一下新调整用药方案对您血糖控制的情况，如门诊沟通，您可以通过护血糖APP手动上传数据，不知道是没有测量还是不会使用手动上传功能呢？'
      contentCode = 'AOP_3'
    } else {
      text =
        '如门诊沟通，您可以通过护血糖APP手动上传数据，我会结合数据帮您分析一下饮食、运动、药物存在的问题，但最近没有收到您血糖记录，不知道是没有测量还是不会使用手动上传功能呢？'
      contentCode = 'AOP_4'
    }
  }
  return {
...
const getChatRoomId = async patientId => {
  ...
  let chatRoomIdInfo = chatRoomInfos.filter(o => {
    return (
      o.participants[0].userId === patientId ||
      o.participants[1].userId === patientId
    )
  })
