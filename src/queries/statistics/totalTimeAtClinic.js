export const totalTimeAtClinic = async (_, args, { getDb }) => {
  ...
  const averages = await db
    ...
    .aggregate([
      {
        $match: {
          'timing.checkIn': { $exists: 1 },
          'timing.print': { $exists: 1 },
        },
      },
      {
        $project: {
          appointmentTime: 1,
          assessmentTimeInMins: {
            $divide: [
              { $subtract: ['$timing.print', '$timing.checkIn'] },
              60 * 1000,
            ],
          },
        },
      ...
      { $unwind: '$assessmentTimesInMins' },
      {
        $group: {
          _id: '$_id',
          averageAssessmentTimeInMins: { $avg: '$assessmentTimesInMins' },
        },
      },
  ...
  return averages.map(
    ({ _id: { year, month, day }, averageAssessmentTimeInMins }) => {
