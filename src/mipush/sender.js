...
export const sender = async ({
  ...
  if (
    dataFromUserCollection &&
    dataFromUserCollection.deviceContext.systemName
  ) {
    const systemName = get(
      dataFromUserCollection,
      'deviceContext.systemName',
      'android',
    ).toLowerCase()
    ...
    console.log(
      'MIPush 异常：未在 users 表中查询到该 patientId 对应的设备信息，查询 bloodGlucoses 表。patientId = ',
      pushId,
    )

    if (
      dataFromBGCollection &&
      dataFromBGCollection.deviceInformation.bundleId
    ) {
      const bundleId = get(
        dataFromBGCollection,
        'deviceInformation.bundleId',
        'com.ihealth.HuTang',
      )
      await realSender({
        type,
      ...
      console.log(
        'MIPush 异常：同时未在 bloodGlucoses 表中查询到测量历史，无法确定设备类型，不进行消息推送。patientId = ',
        pushId,
      )
    }
  }
