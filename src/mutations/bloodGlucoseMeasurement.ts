...
import {addDelayEvent, deleteDelayEvent, queryDelayEvent} from '../redisCron/controller'
export const saveBloodGlucoseMeasurement = async(_, args, {getDb} : IContext,) => {
  ...
  const dinnerSituation = Object
    .entries(DigestiveStateLookup)
    .find(([key, value]) => value === digestiveState,)![0]
...
export const saveBloodGlucoseMeasurementNew = async(_, args, {getDb} : IContext,) => {
  const db = await getDb()
  const {
    bloodGlucoseValue,
  ...
  // 餐前血糖测量后添加定时事件 只有自动测量和餐前才添加定时事件 添加事件前清除这个患者之前的事件
  // 事件key：pigeon_bg_<patientId>_BEFORE_BREAKFAST_午餐前_hh:mm if (inputType ===
  // 'DEVICE') {   if (     measurementTime.indexOf('BEFORE') > -1 &&
  // measurementTime != 'BEFORE_SLEEPING'   ) {     console.log('-----------add
  // reids cron job----------------')     const key =
  // `bg_${patientId}_${measurementTime}_${
  // measureTimeChinese[measureTimeEng.indexOf(measurementTime)]
  // }_${moment().format('HH:mm')}`     const querykey = `bg_${patientId}` const
  // existKeys = await queryDelayEvent(querykey)     console.log('*****',
  // existKeys)     if (existKeys && existKeys.length > 0) {
  // existKeys.forEach(async element => {         await deleteDelayEvent(element)
...
export const updateRemarkOfBloodglucoses = async(_, args, {getDb} : IContext,) => {
  const db = await getDb()
...
export const updateRemarkOfBloodglucosesNew = async(_, args, {getDb} : IContext,) => {
  const db = await getDb()
