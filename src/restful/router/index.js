...
// restfulRouter.get('/wechatSandbox', async ctx => {   const result = await
// wechatPayServices.createUnifiedOrder({     data: {       totalPrice: 2.01,
//    patientId: 'test123',     },   })   ctx.body = result })
// restfulRouter.get('/wechatSandbox/query', async ctx => {   const { orderId }
// = ctx.query   const result = await wechatPayServices.queryUnifiedOrder({
// orderId,     type: 'out_trade_no',   }) })
// restfulRouter.post('/sendMassText', async ctx => {   const result = await
// sendMassText(ctx)   ctx.body = result })
